...
./venv/bin/python refactored.py --client g2s --input-csv parts.csv --output-csv results.csv
...
### Core Components
- **`refactored.py`**: Main entry point with client selection, processing loop, progress tracking
...
python refactored.py --client g2s

# With all options
python refactored.py --client g2s --input-csv parts.csv --output-csv results.csv
```
...
- Quick “what does what” cheatsheet:
  - `refactored.py`: Main entry; wires config, logging, proxy test, processing loop, saving, summary, email.
